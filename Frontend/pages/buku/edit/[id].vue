<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    <form
      @submit.prevent="updateFormDataBuku"
...
export default {
  ...
  mounted() {
    ...
    axios
      .get(`http://localhost:8000/api/buku/${id}`)
      .then((response) => {
        this.post.judul = response.data.data.judul;
        this.post.penulis = response.data.data.penulis;
        this.post.penerbit = response.data.data.penerbit;
        this.post.tahun_terbit = response.data.data.tahun_terbit;
        this.post.isbn = response.data.data.isbn;
        this.post.jumlah_halaman = response.data.data.jumlah_halaman;
        this.post.kategori = response.data.data.kategori;
        this.post.sinopsis = response.data.data.sinopsis;
        this.post.cover_buku = response.data.data.cover_buku;
  ...
  methods: {
    ...
    async updateFormDataBuku(e) {
      ...
      // Periksa apakah cover_buku adalah File baru atau string path
      if (this.post.cover_buku instanceof File) {
        formData.append("cover_buku", this.post.cover_buku);
      }

      // Untuk Laravel yang menggunakan form method spoofing
      formData.append("_method", "PUT");

      try {
        await axios
          .post(`http://localhost:8000/api/buku/${id}`, formData, {
            headers: {
